...
- [~] Check installer and portable builds on a clean Windows PC. Fresh-location install and portable launch passed on this Windows host. `npm run verify:windows-release` now automates hashes, signatures, temporary install, bundled-tool checks, and both launch checks; a separate clean PC remains an external release check.
